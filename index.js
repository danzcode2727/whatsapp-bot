...
prefix = ","
name = "D A N Z | B O T"
rdaftar = "TERIMA KASIH SUDAH MENDAFTAR MENJADI MEMBER DANZBOT ^_^"
rmenu = "HAI MEMBER DANZBOT👋 JANGAN LUPA DONASI YA ^_^"
limitt = 30
ban = []
userpremium = ["6289686610207@s.whatsapp.net","6281253491026@s.whatsapp.net","6285338319214@s.whatsapp.net","6282111829914@s.whatsapp.net"] //ubah nomer kalian
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ZidanGans = ["6289686610207@s.whatsapp.net"] // ubah aja gapapa
			const zidan = mek.message.conversation
		...
		case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('kamu sudah Menjadi Member DanzBot ^_^')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (isBanned) return reply(mess.only.benned)
				...
				ben = '```List Banned``` :\n'
					...
					break 
				case 'premiumlist':
				prem = '```List User Premium D A N Z  B O T``` :\n'
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 D A N Z  B O T〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat DANZBOT\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
					...
					break
				case 'bc':
					if (!isOwner) return reply('Kamu Siapa? Lol')
					if (args.length < 1) return reply('.......')
					anu = await client.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *DANZBOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
		...
		case 'happymod':
			...
			break
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Tidak Terdaftar Di Dalam Database.. Silahkan ketik *${prefix}menu* untuk menampilkan fitur bot ^_^`)
                  }
